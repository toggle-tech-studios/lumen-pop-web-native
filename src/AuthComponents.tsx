...
export function AuthScreen({ onComplete }: { onComplete: (username: string) => void }) {
  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // state observer will catch this and advance to next step
    } catch (err: any) {
  ...
  const handleUsernameSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (rtdb) {
        ...
        await dbSet(dbRef(rtdb, `users/${u.uid}`), {
          username: clean,
          email: u.email || '',
          coins: 0,
          highestLevel: 1,
      ...
      setError('Network error saving username.');
      setLoading(false);
    }
  };
    ...
    <div className="center-screen game-shell">
      ...
      <div className="stagger relative flex max-w-sm w-full mx-4 flex-col items-center text-center p-8 bg-[#12053c]/90 rounded-[2.5rem] border border-white/15 shadow-2xl backdrop-blur-md">
        <div className="sparkle mb-4 w-16 h-16 rounded-full bg-cyan-500/10 border border-cyan-400/30 flex items-center justify-center">
          {step === 'auth' ? (
            <ShieldCheck size={32} className="text-cyan-300 drop-shadow-[0_0_12px_#00f0ff]" />
          ) : (
        ...
        <h1 className="display text-3xl font-extrabold text-white mt-1 mb-2">
          {step === 'auth' ? 'Lumen Pop' : 'Claim Your Name'}
        </h1>
        <p className="text-white/65 text-xs mb-6 max-w-xs">
          {step === 'auth' 
            ? 'Sign in or create an account to save your cosmic journey.' 
            : 'Pick a unique stargazer username.'}
        </p>

        {error && <p className="text-pink-300 text-xs mb-4 px-2 py-1.5 rounded bg-pink-900/30 border border-pink-500/20 w-full">{error}</p>}
        
        {loading ? (
           <div className="py-8"><div className="w-8 h-8 rounded-full border-2 border-cyan-400 border-t-transparent animate-spin mx-auto"></div></div>
        ) : step === 'auth' ? (
          mode === 'menu' ? (
            <div className="flex flex-col gap-3 w-full">
              <button onClick={handleGoogleSignIn} className="btn-soft w-full flex items-center justify-center gap-3 bg-white text-gray-900 hover:bg-gray-100 font-bold py-3 shadow-md">
              ...
              <button onClick={() => setMode('email-sign-in')} className="btn-soft w-full flex items-center justify-center gap-2 py-3 text-sm font-semibold">
                <Mail size={16} /> Continue with Email
              </button>
            </div>
          ) : (
            <form onSubmit={handleEmailAuth} className="flex flex-col gap-3 w-full">
              <input type="email" placeholder="Email address" value={email} onChange={e => setEmail(e.target.value)} className="w-full bg-black/40 border border-white/20 rounded-xl px-4 py-3 text-white placeholder-white/40 focus:outline-none focus:border-cyan-300 text-sm" required />
              <input type="password" placeholder="Password (min 6 chars)" value={password} onChange={e => setPassword(e.target.value)} className="w-full bg-black/40 border border-white/20 rounded-xl px-4 py-3 text-white placeholder-white/40 focus:outline-none focus:border-cyan-300 text-sm" required minLength={6} />
              <button type="submit" className="btn-primary w-full mt-1 py-3 text-sm font-bold">
                {mode === 'email-sign-up' ? 'Create Account' : 'Sign In'}
              </button>
              <button type="button" onClick={() => { setMode(mode === 'email-sign-up' ? 'email-sign-in' : 'email-sign-up'); setError(''); }} className="text-xs text-cyan-300 mt-1 hover:underline text-center">
                {mode === 'email-sign-up' ? 'Already have an account? Sign In' : "Need an account? Sign Up"}
              </button>
              <button type="button" onClick={() => { setMode('menu'); setError(''); }} className="text-xs text-white/50 mt-2 hover:text-white flex items-center justify-center gap-1.5">
                ← Back to options
              </button>
            </form>
          )
        ) : (
          <form onSubmit={handleUsernameSubmit} className="w-full flex flex-col items-center gap-3">
            <input 
              type="text" 
              ...
              autoFocus
            />
            <button type="submit" className="btn-primary flex items-center justify-center gap-2 w-full mt-2 py-3 text-sm font-bold shadow-[0_0_20px_rgba(255,187,66,0.3)]">
